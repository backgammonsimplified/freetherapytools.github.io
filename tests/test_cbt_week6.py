...
class CbtWeek6Tests(unittest.TestCase):
    ...
    def test_original_downloads(self):
        ...
        for ext in ["pdf", "docx"]:
            self.assertIn(f"*.{ext} binary", attributes)
            resource = f"site/resources/free-therapy-tools/cbt/avoidance-and-approach-planner.{ext}"
            actual = subprocess.check_output(
                ["git", "check-attr", "text", "diff", "merge", "--", resource],
                cwd=ROOT, text=True,
            )
            for attribute in ["text", "diff", "merge"]:
                self.assertIn(f"{resource}: {attribute}: unset", actual)
