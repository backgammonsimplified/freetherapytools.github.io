...
```text
C:\path\to\freetherapytools.github.io
```
...
```text
C:\path\to\personal-planning-reference
```
...
```bash
cd /c/path/to/freetherapytools.github.io
...
"C:/path/to/freetherapytools.github.io/.git" = "write"
"C:/path/to/backgammon-reference" = "read"
"C:/path/to/personal-planning-reference" = "read"
"C:/path/to/source-scans" = "read"
"C:/path/to/source-scans" = "read"
...
```powershell
Set-Location "C:\path\to\freetherapytools.github.io"

codex `
  --ask-for-approval never `
  -c 'default_permissions="therapy-skill-kit"' `
  -c 'permissions.therapy-skill-kit.extends=":workspace"' `
  -c 'permissions.therapy-skill-kit.filesystem."C:/path/to/freetherapytools.github.io/.git"="write"' `
  -c 'permissions.therapy-skill-kit.filesystem."C:/path/to/personal-planning-reference"="read"' `
...
```text
C:\path\to\source-scans\dbt_skills_training_handouts_and_worksheets_-_linehan_marsha_srg_.pdf
```

Older high-resolution source:

```text
C:\path\to\source-scans\php.pdf
```
...
```bash
git -C /c/path/to/backgammon-reference \
...
```bash
git -C /c/path/to/personal-planning-reference \
...
The private authoring source `3-mindfulness.pdf` is a 104-page Mindfulness
program scan. It and its manually reviewed OCR/page map belong under ignored
`tmp/source-scans/` and `tmp/source-notes/`; neither file may be committed,
copied into the public site, or required by a production build. Pages 1-102 map
into the existing twelve-lesson Mindfulness curriculum: Introduction & States
of Mind; Observe; Describe; Participate; Non-Judgmentally; One-Mindfully;
Effectively; Self-Compassion & Loving Kindness; Mindfulness of Emotions;
Mindfulness of Thoughts; Grounding; and Being & Doing Mind. Page 103 begins
Interpersonal Effectiveness and is outside this curriculum; page 104 is blank.

`site/data/mindfulness-source-audit.json`, generated by
`scripts/mindfulness_source_audit.py`, is the durable authority for both the
existing DBT book source and this program scan. It preserves the earlier DBT
book-page records and adds all 104 program pages, twelve session ranges,
structural/boundary exclusions, original public page assets, and verified exact
matches. An exact clean match requires the handout or worksheet number, exact
title, internal wording, and layout to agree. Similar-looking material is not
enough. The older program scan is the same low-resolution binary and therefore
is not a clean source.

For substantive official matches, Learn presents **Original Source** first and
the exact higher-resolution **Clean Printable Copy** immediately below it.
Original extracts live under `site/resources/mindfulness/program-source/`;
verified DBT-book extracts live under `site/resources/clean/mindfulness/`.
Reuse these assets instead of re-extracting or upscaling them. Where no exact,
redistributable higher-resolution source exists, publish original adapted HTML
and record the unavailable clean match. Third-party supplementary infographics
are not published without confirmed permission.

The existing `/tool-finder/emotions/` route defaults to **Identify from clues**
and keeps its force graph under the secondary **Explore an emotion** mode. Its
seven clue categories are derived at runtime from
`site/data/skill-apps/emotions.json`: words, body changes, prompting events,
interpretations, action urges, expressions/actions, and aftereffects. Duplicate
clues may map to more than one of the ten emotion families. For each emotion,
the rough match is `round(matched selected clues / total selected clues * 100)`;
percentages are independent, need not total 100, and are never probabilities,
scores, or diagnoses. Saved legacy `step`, `emotion`, `words`, `regions`, and
`details` data is normalized into the shared progress state and remains present
in readable exports.
...
Focused regressions are in `tests/test_mindfulness_emotion_pass.py`,
