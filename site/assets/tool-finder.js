(function () {
  ...
  const entryOverrides = {
    tipp: {
      kind: "tool",
      tool_type: "worksheet",
      tool_href: "/tool-finder/tipp/",
      learn_href: "/learn/distress-tolerance/tipp.html",
      summary: "Practice Temperature, Intense Exercise, Paced Breathing, and Progressive Muscle Relaxation."
    },
    temperature: {
      learn_href: "/learn/distress-tolerance/temperature.html"
    },
    "intense-exercise": {
      learn_href: "/learn/distress-tolerance/intense-exercise.html"
    },
    "paced-breathing": {
      learn_href: "/learn/distress-tolerance/paced-breathing.html"
    },
    pmr: {
      learn_href: "/learn/distress-tolerance/progressive-muscle-relaxation.html"
    }
  };

  const withOverrides = (entry) => ({ ...entry, ...(entryOverrides[entry.id] || {}) });
  ...
  async function init() {
    ...
    const entries = catalogue.entries.map(withOverrides);
    ...
    function render() {
      ...
      host.innerHTML = catalogue.topics.map((topic) => {
        const topicEntries = entries.filter((entry) => !entry.featured_on_home && entry.official_topic === topic && (kind === "all" || entry.kind === kind) && (!query || searchable(entry).includes(query)));
        shown += topicEntries.length;
        return topicEntries.length ? `<section class="tool-finder-topic" data-tool-finder-topic="${esc(topic)}"><h2>${esc(topic)} Tools</h2><div class="tool-finder-grid">${topicEntries.map(card).join("")}</div></section>` : "";
      }).join("");
