(function (global) {
  ...
  function flowSummary(flow, state) {
    if (flow.id === "dime-game") {
      const mode = state.answers.mode === "say-no" ? "Saying No" : "Asking";
      const countedAnswer = state.answers.mode === "say-no" ? "no" : "yes";
      const factors = flow.nodes.filter((node) => node.dime_factor);
      const count = factors.filter((node) => state.answers[node.field] === countedAnswer).length;
      return Progress.nonEmptySections("The Dime Game", [
        ["Situation", state.answers.situation], ["Decision", mode],
        ["Dime Count", `${count} dimes (${count * 10}¢)`],
        ["Factor Answers", factors.map((node) => `${node.dime_factor}: ${state.answers[node.field] || "Not answered"}`)],
      ]);
    }
  ...
  class ConstrainedTreeEngine {
    ...
    dimeResultMarkup() {
      const mode = this.answers.mode === "say-no" ? "say-no" : "ask";
      const count = this.flow.nodes.filter((node) => node.dime_factor).filter((node) => this.answers[node.field] === (mode === "ask" ? "yes" : "no")).length;
      const ask = ["Don't ask; don't hint.", "Don't ask; don't hint.", "Hint indirectly; take no.", "Hint openly; take no.", "Ask tentatively; take no.", "Ask gracefully, but take no.", "Ask confidently; take no.", "Ask confidently; resist no.", "Ask firmly; resist no.", "Ask firmly; insist; negotiate; keep trying.", "Don't take no for an answer."];
      const sayNo = ["Do it without being asked.", "Do it without being asked.", "Don't complain; do it cheerfully.", "Do it, even if you're not cheerful about it.", "Do it, but show that you'd rather not.", "Say you'd rather not, but do it gracefully.", "Say no firmly, but reconsider.", "Say no confidently; resist saying yes.", "Say no firmly; resist saying yes.", "Say no firmly; resist; negotiate.", "Don't do it."];
      return `<section class="dime-result"><p class="dime-total"><strong>${count} dime${count === 1 ? "" : "s"}</strong> (${count * 10}¢)</p><h4>${escapeHtml((mode === "ask" ? ask : sayNo)[count])}</h4><p>Review the list before acting. If some factors are much more important than others, check Wise Mind and adjust how strongly you respond.</p></section>`;
    }

    currentMarkup(node) {
      if (node.type === "result") return `<article class="skill-guided-step is-current is-result" data-guided-current tabindex="-1"><p class="skill-tree-kicker">Result</p><h3>${escapeHtml(node.title)}</h3><p>${escapeHtml(node.body || "")}</p>${node.dynamic_result === "dime-game" ? this.dimeResultMarkup() : this.dynamicResult(node)}${node.calendar ? `<details class="skill-tree-calendar"><summary>${escapeHtml(node.calendar.label)}</summary><div data-tree-calendar></div></details>` : ""}${linkCards(node.links)}</article>`;
      const choices = node.dynamic_choices === "emotions" ? this.context.emotions.map((emotion) => ({ label: emotion.name, value: emotion.id, next: node.next })) : node.choices || [];
      const textEditor = node.control === "text" ? `<form data-tree-text><label for="tree-${escapeHtml(node.field)}">${escapeHtml(node.help || "Write only what is useful here.")}</label><textarea id="tree-${escapeHtml(node.field)}" name="answer">${escapeHtml(this.answers[node.field] || "")}</textarea><button type="submit">Continue</button></form>` : "";
      const information = node.type === "information" ? `<p>${escapeHtml(node.body || "")}</p><button type="button" data-tree-continue>Continue</button>` : "";
